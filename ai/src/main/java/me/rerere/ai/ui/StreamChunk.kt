...
sealed class StreamChunk {
    ...
    data class Finish(
         ...
         *
         * 终止信号各协议不同：OpenAI ChatCompletions 是 `data: [DONE]`，
         * Response API 是 `response.completed` / `response.incomplete`，
         * Claude 是 `message_stop` 事件。Google Gemini 的 SSE 不发终止信号、靠 EOF 收尾，
         * 因此该标记对 Google 恒为 true —— 上层判定必须同时要求 finishReason 为空，
         * 才不会把 Google 的正常收尾误判成截断。
         */
        val terminatedWithoutSentinel: Boolean = false,
    ) : StreamChunk()
}
