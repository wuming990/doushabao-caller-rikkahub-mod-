...
data class UIMessage(
    ...
    /**
     * v297：这条回答**真实花掉**的 token 累计（同一趟生成里每一次请求相加）。
     *
     * 与上面的 [usage] 是两个口径，都必须留着：
     * - `usage` 是「上下文水位」—— 最后一次请求的输入量，自动压缩和输入框那条进度条靠它；
     * - `cumulativeUsage` 是「真实消耗」—— 每调用一次工具、每自动续跑一次，都会把全量上下文
     *   重新发一遍并重新计费。旧实现只有覆盖语义的 usage，于是「一条回答调了 5 次工具」
     *   在统计里只算最后那次，用户看到的数字比实际花费小好几倍。
     *
     * 累加逻辑见 GenerationLoop 的 foldRoundUsage；「重新生成最后一次输出」丢掉的半截
     * 也算在内 —— 那一次请求确实发出去了、也计费了，不算就是少报。
     * 新字段带默认值，旧聊天 JSON/数据库记录仍可正常反序列化，无需迁移。
     */
    val cumulativeUsage: TokenUsage? = null,
    val translation: String? = null,
    // 请求期间生成的内部消息；该标记仅在内存中使用
    @Transient
    val isSynthetic: Boolean = false,
    /**
     * Provider 返回的结束原因（例如 stop/end_turn/length/max_tokens/incomplete:max_output_tokens）。
     *
     * v221：此前该值在 StreamChunkHandler 中被丢弃，导致上层无法区分「完整结束」与
     * 「达到输出上限被截断」，子代理/圆桌只能把半截结果误判成成功。
     * 新字段带默认值，旧聊天 JSON/数据库记录仍可正常反序列化，无需迁移。
     */
    val finishReason: String? = null,
    /**
     * v269：本次响应流是否在「从未收到协议终止信号」的情况下结束
     * （OpenAI 的 [DONE] / Claude 的 message_stop / Response API 的 response.completed）。
     *
     * 这是识别「上游无声掐断」的一半依据，单独这一条不足以判定截断 ——
     * Google Gemini 的 SSE 本身不发终止信号，正常收尾时该值也为 true。
     * 必须与 finishReason 为空同时成立，判定逻辑见 GenerationLoop 的续跑分支。
     * 新字段带默认值，旧聊天 JSON/数据库记录仍可正常反序列化，无需迁移。
     */
    val truncatedWithoutSentinel: Boolean = false,
    /**
     * v269：这条回答累计被自动续跑了几次（长度上限截断 + 异常中断共用同一计数）。
     *
     * 0 = 没续跑过。随消息一起落库，重开 App 后界面仍能显示「续跑 N 次」；
     * 显示由 DisplaySetting.showResumeCount 控制，且仅在本值 > 0 时才渲染。
     * 新字段带默认值，旧聊天 JSON/数据库记录仍可正常反序列化，无需迁移。
     */
...
/**
 * v274：Gemini 官方账号（Cloud Code Assist 通道）专用的单事件块形状。
 *
 * 这是「遗留 chunk 形状」，仅为 [me.rerere.ai.provider.providers.google.GoogleProvider.parseStreamCandidates]
 * 保留 —— Gemini OAuth 通道（app 模块的 GeminiProvider）跨模块消费它。新的流式代码应使用
 * [StreamChunk]；不要为这两个类型新建生产者或消费者。
 * （照搬自 ExTV rikkahub-agent，原注释同义）
 */
@Serializable
