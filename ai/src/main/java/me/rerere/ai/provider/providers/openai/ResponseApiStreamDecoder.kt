...
internal class ResponseApiStreamDecoder : StreamChunkDecoder {
    ...
    // v269：走到这里说明上游没发 response.completed / response.incomplete / [DONE] 就断了
    override fun onClosed(): List<StreamChunk> = state.finish(terminatedWithoutSentinel = true)
    ...
    private class ResponseStreamState {
        ...
        fun finish(
            ...
            return buildList {
                ...
                add(StreamChunk.Finish(finishReason, responseId, model, terminatedWithoutSentinel))
            }
        }
    }
}
