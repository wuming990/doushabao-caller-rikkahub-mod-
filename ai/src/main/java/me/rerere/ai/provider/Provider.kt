...
data class CustomBody(
    ...
    /**
     * Legacy whitelist from the first mod2 build. Settings migration converts it
     * into [excludedProviderIds]; new code paths must not write this field.
     */
    val providerIds: Set<Uuid>? = null,
    /**
     * Exclusion list (current semantics):
     * null / empty = send to every provider;
     * non-empty = do NOT send to the listed providers.
     */
    val excludedProviderIds: Set<Uuid>? = null,
)
