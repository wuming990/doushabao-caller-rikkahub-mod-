...
class WorkspaceManager(
    ...
    /** 创建目录（包括空目录），路径始终限制在对应工作区存储区内。 */
    fun ensureDirectory(
        root: String,
        path: String,
        area: WorkspaceStorageArea = WorkspaceStorageArea.FILES,
    ): Boolean {
        val directory = fileSystem.resolve(areaDir(root, area), path)
    ...
    fun glob(
        ...
        fileSystem.glob(filesDir(root), pattern, path, report, budgeted)
    ...
    fun grep(
        ...
        fileSystem.grep(filesDir(root), query, path, regex, ignoreCase, includeGlob, report, budgeted)
