...
class ExampleUnitTest {
    ...
    @Test
    fun commandCanRunWithoutTimeout() {
        val baseDir = Files.createTempDirectory("workspace-unlimited-command-test").toFile()
        val manager = WorkspaceManager(baseDir)
        val root = "test-workspace"
        manager.ensureWorkspace(root)

        val result = manager.executeCommand(root, "sleep 1 && printf done", timeoutMillis = 0L)
