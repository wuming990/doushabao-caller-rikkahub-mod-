...
fun GrokProviderConfigure(
    ...
    // v282：导入外部工具导出的凭据。给「只有钥匙、没有账号密码」的号用 —— 那种号走不了
    // 设备码登录（登录要在网页上点批准），只能把凭据搬进来。
        ...
        scope.launch {
            try {
                ...
                if (content.isNullOrBlank()) {
                    toaster.show(
                        context.getString(R.string.grok_import_empty),
                        type = ToastType.Error,
                    )
                    return@launch
                }
                val result = repository.importCredentials(content)
                if (result.total == 0) {
                    toaster.show(
                        context.getString(R.string.grok_import_empty),
                        type = ToastType.Error,
                ...
                // 导入完顺手把模型列表拉一次，省得用户还要手动点一下才看到可用模型。
                // 失败不影响导入本身（凭据已经落库），所以只吞这一处的错。
                runCatching {
                ...
                if (result.skipped > 0) {
                    toaster.show(
                        context.getString(R.string.grok_import_skipped, result.skipped),
                        type = ToastType.Warning,
                    )
                }
            } catch (cancel: CancellationException) {
                // 页面销毁导致的取消原样上抛，不能当成导入失败弹一个错。
                throw cancel
            } catch (error: Throwable) {
                toaster.show(
                    context.getString(R.string.grok_import_failed, error.message.orEmpty()),
                    type = ToastType.Error,
                )
            }
        }
    ...
    Column(
        ...
        OutlinedButton(
            ...
            Text(stringResource(R.string.grok_import_credentials))
        }

        Text(
            text = stringResource(R.string.grok_import_hint),
...
private fun GrokAccountCard(
    ...
    Card(
        ...
        Column(
            ...
            //
            // 这个额度没有接口能查，数字只能从「免费额度用尽」的报错里学到，所以**必须把观测时刻
            // 一起显示**，否则就是把「面板满格骗人」换成「面板显示旧数字骗人」。没撞过墙就不显示。
            val modelLimits = account.usage?.modelLimits.orEmpty()
            if (modelLimits.isNotEmpty()) {
                Text(
                    text = stringResource(R.string.grok_model_limit_title),
                    style = MaterialTheme.typography.labelMedium,
                )
                modelLimits.sortedByDescending { it.observedAt }.forEach { limit ->
                    Text(
                        text = stringResource(
                            R.string.grok_model_limit_value,
                            limit.model,
                            formatTokenCount(limit.usedTokens),
                            formatTokenCount(limit.limitTokens),
                        ) + " · " + stringResource(
                            R.string.grok_model_limit_observed,
                            GROK_RESET_FORMAT.format(Instant.ofEpochMilli(limit.observedAt)),
                        ),
                        style = MaterialTheme.typography.bodySmall,
                        color = if (limit.usedTokens >= limit.limitTokens) {
            ...
            if (account.cooldownUntil > System.currentTimeMillis()) {
                Text(
                    text = stringResource(
                        R.string.grok_cooldown_until,
                        GROK_RESET_FORMAT.format(Instant.ofEpochMilli(account.cooldownUntil)),
                    ),
                    style = MaterialTheme.typography.bodySmall,
                    color = MaterialTheme.colorScheme.error,
                )
            ...
            Text(
                text = stringResource(R.string.grok_model_limit_note),
                style = MaterialTheme.typography.labelSmall,
                color = MaterialTheme.colorScheme.onSurfaceVariant,
            )
...
/** v284：token 数按千分位显示（672022 → 672,022），比「万」这种单位跨语言更稳。 */
private fun formatTokenCount(tokens: Long): String = "%,d".format(tokens)
