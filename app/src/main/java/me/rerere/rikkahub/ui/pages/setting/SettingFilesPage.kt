...
fun SettingFilesPage(
    ...
    // 选择集合只按当前列表里真实存在的 id：列表中途新增/删除会自动清理无效选择，
    // 新出现的图片永远不会被「全选」或「删除所选」误删；确认删除时再冻结快照。
    val visibleIds = remember(files) { files.mapTo(mutableSetOf()) { it.id } }
    val visibleSelectedIds = remember(selectedIds, visibleIds) {
        selectedIds.filterTo(mutableSetOf()) { it in visibleIds }
    }
    ...
    if (pendingBatchDelete != null) {
        ...
        AlertDialog(
            ...
            text = { Text(stringResource(R.string.setting_files_page_batch_delete_confirm, batchIds.size)) },
            confirmButton = {
                TextButton(
                    onClick = {
                        pendingBatchDelete = null
                        scope.launch {
                            val result = filesManager.deleteBatch(batchIds)
                            if (result.failedIds.isEmpty()) {
                                toaster.show(
                                    context.getString(R.string.setting_files_page_batch_deleted_toast, result.deletedIds.size)
                                )
                            } else {
                                toaster.show(
                                    context.getString(
                                        R.string.setting_files_page_batch_partial_failed_toast,
        ...
        topBar = {
            LargeFlexibleTopAppBar(
                title = {
                    Text(
                        if (selectionMode) {
                            stringResource(R.string.setting_files_page_selected_count, visibleSelectedIds.size)
                        } else {
                            stringResource(R.string.setting_files_page_title)
                        }
                    )
                },
                ...
                actions = {
                    if (selectionMode) {
                        TextButton(
                            onClick = {
                                selectedIds = if (visibleSelectedIds.size == visibleIds.size) {
                                    emptySet()
                                } else {
                                    visibleIds
                                }
                            ...
                            Text(
                                stringResource(
                                    if (visibleSelectedIds.size == visibleIds.size) {
                                        R.string.setting_files_page_clear_selection
                                    } else {
                                        R.string.setting_files_page_select_all
                                    }
                                )
                            )
                        }
                        TextButton(
                            onClick = { pendingBatchDelete = visibleSelectedIds.toSet() },
                            enabled = visibleSelectedIds.isNotEmpty(),
                        ) {
                            Text(stringResource(R.string.setting_files_page_delete_selected))
                        }
                        TextButton(
                            onClick = {
                                selectionMode = false
                            ...
                            Text(stringResource(R.string.setting_files_page_exit_selection))
                        }
                    } else {
                        TextButton(
                            ...
                            Text(stringResource(R.string.setting_files_page_select_action))
                        }
                        IconButton(
                ...
                LazyVerticalStaggeredGrid(
                    ...
                    items(files, key = { it.id }) { file ->
                        FileItem(
                            ...
                            selected = file.id in visibleSelectedIds,
                            onToggle = {
