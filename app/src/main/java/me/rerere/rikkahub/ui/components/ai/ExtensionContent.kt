...
fun ModeInjectionsContent(
    ...
    onManage: (() -> Unit)? = null,
    currentModelId: kotlin.uuid.Uuid? = null,
) {
    LazyColumn(
        ...
        items(modeInjections) { injection ->
            ...
            val followsModel = automaticTargetModelIds.isNotEmpty()
            val modelMatches = currentModelId != null && currentModelId in automaticTargetModelIds
            val actuallyActive = injection.enabled && if (followsModel) {
                modelMatches
            } else {
                selectedIds.contains(injection.id)
            }
            ListItem(
                headlineContent = {
                    Text(injection.name.ifBlank { stringResource(R.string.extension_content_unnamed) })
                },
                supportingContent = if (followsModel || !injection.enabled) {
                    {
                        Text(
                            stringResource(
                                when {
                                    !injection.enabled -> R.string.extension_content_disabled_in_settings
                                    modelMatches -> R.string.extension_content_model_auto_on
                                    else -> R.string.extension_content_model_auto_off
                                }
                            ),
                ...
                trailingContent = {
                    Switch(
                        checked = actuallyActive,
                        enabled = injection.enabled,
                        onCheckedChange = if (followsModel || !injection.enabled) {
                            null
                        } else {
                            { checked -> onToggle(injection.id, checked) }
                        },
                    )
                },
