...
fun ProviderConfigure(
    ...
    Column(
        ...
        when (provider) {
            ...
            // v274：三套官方账号 OAuth 登录在 SettingProviderDetailPage 已被各自的
            // 专属 Configure 提前拦截，正常流程到不了这里；空分支仅为满足穷尽性。
...
internal fun ProviderSetting.defaultBaseUrlForReset(): String {
    ...
    if (defaultProvider != null) {
        when (this) {
            ...
            // v274：账号登录类无 baseUrl 概念
            is ProviderSetting.Codex, is ProviderSetting.Grok, is ProviderSetting.GeminiOAuth -> return ""
        }
    }
    return when (this) {
...
internal fun ProviderSetting.resetBaseUrlToDefault(): ProviderSetting {
    ...
    return when (this) {
        ...
        // v274：账号登录类无 baseUrl，重置为原样返回
        is ProviderSetting.Codex, is ProviderSetting.Grok, is ProviderSetting.GeminiOAuth -> this
    }
}
...
internal fun ProviderSetting.isUsingDefaultBaseUrl(): Boolean {
    val baseUrl = when (this) {
        ...
        is ProviderSetting.Google -> this.baseUrl
        is ProviderSetting.Claude -> this.baseUrl
        // v274：账号登录类无 baseUrl 概念，恒视为「默认」
        is ProviderSetting.Codex, is ProviderSetting.Grok, is ProviderSetting.GeminiOAuth -> return true
    }
