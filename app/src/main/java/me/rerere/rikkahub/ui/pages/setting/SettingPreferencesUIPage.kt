        ...
        LazyColumn(
            ...
            item {
                CardGroup(
                    ...
                    // v269：续跑次数显示开关（用户要求「可以关掉，而不是一直显示」）
                    item(
                        headlineContent = { Text(stringResource(R.string.setting_display_page_show_resume_count_title)) },
                        supportingContent = { Text(stringResource(R.string.setting_display_page_show_resume_count_desc)) },
                        trailingContent = {
                            Switch(
                                checked = displaySetting.showResumeCount,
                                onCheckedChange = {
                                    updateDisplaySetting(displaySetting.copy(showResumeCount = it))
                                }
                            )
                        },
                    )
                    // v270：重点标色插件开关（默认关 = 零注入；打开才让模型知道有这能力）
                    item(
                        headlineContent = { Text(stringResource(R.string.setting_display_page_highlight_key_points_title)) },
                        supportingContent = { Text(stringResource(R.string.setting_display_page_highlight_key_points_desc)) },
                        trailingContent = {
                            Switch(
                                checked = displaySetting.highlightKeyPoints,
                                onCheckedChange = {
                                    updateDisplaySetting(displaySetting.copy(highlightKeyPoints = it))
                                }
                            )
                        },
                    )
                    // v294：强制翻阅 skill 开关（默认关 = 菜单制；打开 = 强制先翻阅一遍入口说明）
                    item(
                        headlineContent = { Text(stringResource(R.string.setting_display_page_force_skill_review_title)) },
                        supportingContent = { Text(stringResource(R.string.setting_display_page_force_skill_review_desc)) },
                        trailingContent = {
                            Switch(
                                checked = displaySetting.forceSkillReview,
                                onCheckedChange = {
                                    updateDisplaySetting(displaySetting.copy(forceSkillReview = it))
                                }
                            )
                        },
                    )
                    item(
                        headlineContent = { Text(stringResource(R.string.setting_display_page_show_context_usage_title)) },
                        supportingContent = { Text(stringResource(R.string.setting_display_page_show_context_usage_desc)) },
                        trailingContent = {
                            Switch(
                                checked = displaySetting.showContextUsage,
                                onCheckedChange = {
                                    updateDisplaySetting(displaySetting.copy(showContextUsage = it))
                                }
                            )
                        },
                    )
                    // v297：本对话 token 累计（顶栏那行小字），与「每条消息下方」是两件事
                    item(
                        headlineContent = { Text(stringResource(R.string.setting_display_page_show_conversation_tokens_title)) },
                        supportingContent = { Text(stringResource(R.string.setting_display_page_show_conversation_tokens_desc)) },
                        trailingContent = {
                            Switch(
                                checked = displaySetting.showConversationTokenStats,
                                onCheckedChange = {
                                    updateDisplaySetting(displaySetting.copy(showConversationTokenStats = it))
                                }
                            )
                        },
                    )
