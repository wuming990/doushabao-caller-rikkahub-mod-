...
data class Assistant(
     ...
     *
     * 与全局 `Settings.enableAgentTools` 是「总闸 + 分闸」关系：**两个都开才生效**。
     * 默认 true，所以升级后行为与旧版（只看全局开关）完全一致，不会有助手突然少了子代理。
     */
    ...
    // 自动压缩使用的模型。旧版默认固定压缩模型，保持兼容。
    val autoCompressModelSource: AutoCompressModelSource = AutoCompressModelSource.FIXED,
    // 自动压缩实际触发值，单位为 token；界面以 K 为单位输入。
    val autoCompressTriggerTokens: Int = 115_200,
    // 旧版字段：上下文窗口大小，达到 90% 时触发。仅用于兼容旧设置迁移。
    @SerialName("autoCompressContextWindow")
    val legacyAutoCompressContextWindow: Int? = null,
)

@Serializable
enum class AutoCompressModelSource {
    CURRENT_CHAT,
    FIXED,
}
...
sealed class PromptInjection {
    ...
    /** null 表示全部已启用模型；非 null 时仅对指定模型注入。 */
    abstract val targetModelId: Uuid?
    ...
    data class ModeInjection(
        ...
        // 旧字段保留用于读取旧版 JSON；新界面和新逻辑使用 targetModelIds。
        @SerialName("targetModelId")
        override val targetModelId: Uuid? = null,
        val targetModelIds: Set<Uuid> = emptySet(),
    ) : PromptInjection()
...
/**
 * 模式注入的自动触发模型集合。旧版只有一个 targetModelId，读取时仍视为单元素集合。
 */
fun PromptInjection.ModeInjection.automaticTargetModelIds(): Set<Uuid> =
    targetModelIds.ifEmpty { targetModelId?.let(::setOf).orEmpty() }
