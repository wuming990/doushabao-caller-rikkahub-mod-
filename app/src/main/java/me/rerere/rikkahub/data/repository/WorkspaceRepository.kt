...
/** Linux 基础环境默认下载地址（与工作台详情页手动安装一致）。 */
...
class WorkspaceRepository(
    ...
    /** 只读 AI 检索：在工作区 files 区按通配符查找文件和目录。 */
    suspend fun globFiles(
        ...
        val workspace = dao.getById(id) ?: return@withContext emptyList()
        manager.ensureWorkspace(workspace.root)
        manager.glob(workspace.root, pattern, path, report, budgeted)
    }

    /** 只读 AI 检索：在工作区 files 区搜索文本内容。 */
    suspend fun grepFiles(
        ...
        val workspace = dao.getById(id) ?: return@withContext emptyList()
        manager.ensureWorkspace(workspace.root)
        manager.grep(workspace.root, query, path, regex, ignoreCase, includeGlob, report, budgeted)
    }
    ...
    suspend fun ensureDirectory(
        ...
        val workspace = dao.getById(id) ?: error("Workspace not found: $id")
        manager.ensureWorkspace(workspace.root)
        manager.ensureDirectory(workspace.root, path, area)
    }
    ...
    suspend fun executeCommand(
        ...
        return runInterruptible(Dispatchers.IO) {
            // 工作区加速模式: 本轮第一条命令会升级为前台服务并等待进程移出后台组,
            // 保证这里 fork 出来的子进程继承到较好的 CPU/IO 分组
