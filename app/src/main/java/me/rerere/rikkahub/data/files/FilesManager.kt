...
class FilesManager(
    ...
    companion object {
        ...
        /** v255：附件限制（对齐主流 AI 应用） */
        const val MAX_ATTACH_COUNT = 20
        const val MAX_ATTACH_IMAGE_BYTES = 20L * 1024 * 1024
        const val MAX_ATTACH_DOC_BYTES = 50L * 1024 * 1024
    }
    ...
    /**
     * 流式写入托管文件: 内容由 [writer] 直接写进目标文件, 不需要先在内存里拼出完整 ByteArray.
     * 用于发布 APK 等几十兆的大文件, 避免一次性占用两倍文件大小的堆内存.
     */
    suspend fun saveManagedFromStream(
        folder: String,
        displayName: String,
        mimeType: String = "application/octet-stream",
    ...
    /**
     * v255：单个附件异步复制到本地上传目录（IO 线程），返回本地 file:// Uri。
     * 用于聊天输入区的附件：先标记「处理中」，复制完成再进输入框；失败抛异常由调用方提示。
     */
    suspend fun copyChatFile(uri: Uri): Uri = withContext(Dispatchers.IO) {
        val dir = context.filesDir.resolve(FileFolders.UPLOAD)
        if (!dir.exists()) {
            dir.mkdirs()
        }
    ...
    /** v255：读取 content:// 或 file:// Uri 的字节大小；读取失败返回 null */
    fun getUriSizeBytes(uri: Uri): Long? = runCatching {
        when (uri.scheme) {
            "file" -> uri.toFile().length()
            else -> context.contentResolver.openAssetFileDescriptor(uri, "r")?.use { it.length }
        }
    }.getOrNull()

    /** v255：附件类型分类（按 MIME 前缀；其余一律当文档收，不强拒） */
    fun classifyAttachment(mime: String?): AttachmentKind = when {
        mime.isNullOrBlank() -> AttachmentKind.DOCUMENT
    ...
    /**
     * v255：附件类型分类结果（聊天输入区用）。
     * 刻意放在类体内 —— 调用方一律写 `FilesManager.AttachmentKind.X`，与限制常量同一处取用。
     */
    ...
    /**
     * v289：按一组 id 批量删除（存储管理多选）。
     *
     * 逐项先删磁盘、再删数据库记录：
     * - 磁盘文件本来就不存在 → 算删除成功；
     * - 磁盘删除失败 → **保留数据库记录**并在结果里报告失败（不复用 delete() 吞错的写法）；
     * - 数据库记录不存在 → 算删除成功（没有残留可删）。
     * 返回逐项结果汇总，调用方按 deleted/failed 提示用户。
     */
    suspend fun deleteBatch(ids: Set<Long>): FileBatchDeleteResult = withContext(Dispatchers.IO) {
        val outcomes = ids.map { id ->
            val entity = repository.getById(id)
            if (entity == null) {
                FileDeleteOutcome(id = id, deleted = true)
            } else {
                val file = getFile(entity)
                val deletedFromDisk = !file.exists() ||
                    runCatching { file.deleteRecursively() }.getOrDefault(false)
                if (deletedFromDisk) {
                    FileDeleteOutcome(id = id, deleted = repository.deleteById(id) > 0)
                } else {
                    FileDeleteOutcome(id = id, deleted = false)
                }
            }
        }
        summarizeBatchDelete(outcomes)
    }
...
/** v289：批量删除中单个文件的结果（逐项） */
data class FileDeleteOutcome(
    val id: Long,
    val deleted: Boolean,
)

/** v289：批量删除的汇总结果 */
data class FileBatchDeleteResult(
    val deletedIds: List<Long>,
    val failedIds: List<Long>,
) {
    val deletedCount: Int get() = deletedIds.size
    val failedCount: Int get() = failedIds.size
    val allSucceeded: Boolean get() = failedIds.isEmpty()
}

/** v289：纯汇总函数，把逐项结果聚合成批量结果（可独立单测，不碰磁盘/Room） */
fun summarizeBatchDelete(outcomes: List<FileDeleteOutcome>): FileBatchDeleteResult {
    val deletedIds = outcomes.filter { it.deleted }.map { it.id }
    val failedIds = outcomes.filterNot { it.deleted }.map { it.id }
    return FileBatchDeleteResult(deletedIds = deletedIds, failedIds = failedIds)
}
